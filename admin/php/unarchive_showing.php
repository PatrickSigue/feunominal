...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($showing_id > 0) {
        ...
        try {
            // Unarchive the showing
            $sql_showing = "UPDATE showings SET archived = 0 WHERE showing_id = ?";
            $stmt_showing = $conn->prepare($sql_showing);
            $stmt_showing->bind_param("i", $showing_id);
            $stmt_showing->execute();
            $stmt_showing->close();

            // Unarchive associated movie tickets
            $sql_tickets = "UPDATE movie_tickets SET archived = 0 WHERE showing_id = ?";
            ...
            // Rollback on error
            $conn->rollback();
            echo json_encode(['success' => false, 'error' => 'Failed to unarchive: ' . $e->getMessage()]);
        }
    } else {
