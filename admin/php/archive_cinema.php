...
// Handle form submission for archiving a cinema
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['cinema_id'])) {
        ...
        try {
            // Archive the cinema
            $stmt1 = $conn->prepare("UPDATE cinemas SET archived = 1 WHERE cinema_id = ?");
            $stmt1->bind_param("i", $cinema_id);
            if (!$stmt1->execute()) {
                throw new Exception("Error archiving cinema.");
            }
